...
def processRequest(network, caller_contract_address, supervisor_address, idRequest):
    ...
    while (retries <= MAX_RETRIES) :
        try:
            ...
            relativeSensorValues = retrieveLatestRelativeSensorValues()
            print("\nNew Relative Sensor Values:")
            break
        except KeyboardInterrupt:
            print("Exiting!!!")
            print("Unable to join FilterEvents and CheckOracleState and checkOracleVariations threads")
            ...
            print(f"\n Error number {retries+1} for retrieveLatestSensorValue function.")
            retries+=1
    while (retries <= MAX_RETRIES) :
        try:
            #FUNCTION setLatestSensorValue
            nonce = network['bc'].eth.getTransactionCount(supervisor_address)
            update_txn = network['contract'].functions.setLatestSensorValue(sensorValue, caller_contract_address, idRequest, relativeSensorValues).buildTransaction({ 'gas': 700000, 'nonce': nonce})
...
def retrieveLatestRelativeSensorValues():
    global relativeSensorFile
    global currentRequest
    with open(relativeSensorFile, "r") as filin:
        line = filin.readlines()
    lastValues = line[currentRequest]
    lastValues = lastValues.split()
    for i in range(len(lastValues)):
        try:
            lastValues[i] = int(lastValues[i])
        except:
            lastValues[i] = 0
    currentRequest+=1
    return lastValues
...
if __name__ == "__main__":
    ...
    oracle_contract_address = contract_address("AsymmetricSensorOracle.json")
    ...
    network['contract'] = network['bc'].eth.contract(address=network['bc'].toChecksumAddress(oracle_contract_address), abi=asymmetric_oracle_abi)
